 ...
 <section class="content">
   <div class="box">
    <div class="box-header">
      <div class="dd">
        <h3 class="box-title">Room Available
        </h3>
      </div>
    ...
    <div class="row">
     <div class="col-sm-3">
      <div class="form-group search-bit">
        <select name="hotel_id" id="hotel_id" class="form-control" onchange="room_type(this.value)" required="required">
      ...
      <div class="col-sm-3">
        <div class="form-group search-bit">
         <select name="room_type_id" class="form-control" id="sel_city" required="required">
...
<?= form_close();?>
 ...
 <?php //var_dump($all_hotels);
     // var_dump($room_types);
?>
<input type="hidden" id="hotell_id" value="<?php if(!empty($hotel_id)){echo $hotel_id;}?>">
<input type="hidden" id="roomm_type_id" value="<?php if(!empty($room_type_id)){echo $room_type_id;}?>">
...
//$end = '2003-09-' . date('t', strtotime($date)); //get end date of month
          ...
          <table class="act">
            <tr>
      <td>
<p class="rooms-booked">avail. room</p>
    
    <p class="rooms-booked">total booked </p>
               ...
               $day_num = date('d', strtotime($date));
                ...
                for($i=0;$i<=count($all_details)-1;$i++){
                  ...
                  if($day_num==$all_details[$i]['day'] && $month==$all_details[$i]['month'] && $year==$all_details[$i]['year']){
                    ...
                    $available_room=$all_details[$i]['available_room'];
                    $booked_room=$all_details[$i]['booked_room'];
                    break;
                   }
                  else{
                    $price_per_day1='';
                ...
                <input type='text' name='room_available' placeholder='room_avbl' class='text-wrap' id='$day_num,$month,$year,1' onchange='rooms_available(this.id,this.value)' value='".$available_room."'>
                <input type='text' name='room_booked' placeholder='rooms_booked' class='text-wrap' id='$day_num,$month,$year,2' onchange='rooms_booked(this.id,this.value)' value='".$booked_room."'> 
...
<script>
  ...
  function price_per_hour(id,value){
   ...
   function rooms_booked(id,value){
     var room_type_id =$("#roomm_type_id").val();
    var hotel_id=$("#hotell_id").val();
    $.ajax({
       url:'rooms_booked',
  ...
  function rooms_available(id,value){
     var room_type_id =$("#roomm_type_id").val();
    var hotel_id=$("#hotell_id").val();
    $.ajax({
       url:'rooms_available',
