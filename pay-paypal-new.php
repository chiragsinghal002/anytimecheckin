...
if(isset($_POST['PaRes']))
{
    ...
    $data=array("service"=>"ECOMAPI_SALE_3DSREQ", "trackid"=>$_REQUEST['trackid'], "paRes"=>$_POST['PaRes'], "carddetails"=>$_REQUEST['carddetails']);
    ...
    { 
        $abc1=$_REQUEST['discounted_price'];
        ...
        $_POST['room_type_id']=$_REQUEST['hotel_room_type_id'];
        $_POST['user_id']=$user_id;
        $_POST['paymentId']='PAYPAL12345';
        $_POST['discount_price']=$_REQUEST['discounted_price'];
        $_POST['hotel_booking_id']=$resultbooking;
        $_POST['check_in_date']=date('Y-m-d',strtotime($_REQUEST['check_in_date']));
        $_POST['check_out_date']=date('Y-m-d',strtotime($_REQUEST['check_out_date']));
        $_POST['check_in_time']=date('H:i:s',strtotime($_REQUEST['check_in_time']));
        $_POST['check_out_time']=date('H:i:s',strtotime($_REQUEST['check_out_time']));
        $_POST['noofroom']=$_REQUEST['noofroom'];
        $_POST['no_of_person']=$_REQUEST['no_of_person'];
        $_POST['actual_price']=$_REQUEST['total_price'];
            // var hotel_booking_id=$("#hotel_booking_id").val();
        $_POST['booking_type']= $optradio;
        $_POST['childs']=$_REQUEST['childs'];
        $_POST['fname']=$_REQUEST['fname'];
        $_POST['lname']=$_REQUEST['lname'];
        $_POST['email']=$_REQUEST['email'];
        $_POST['mob_no']=$_REQUEST['mob_no'];
  ...
  if(isset($_POST['submit']))
  {
  	...
  	if(isset($fresult['responseData']['paReq']))
  	{
  		if($fresult['responseData']['paReq']!='')
  		{
  			...
  			<input type="hidden" name="TermUrl" id="TermUrl" value="https://anytimecheckin.com/new/pay-paypal-new.php"/>
