<?php
	...
	class Users extends MY_Controller {
		...
		public function index(){
			...
			{
			    	$data['all_users'] =  $this->user_model->get_assign_list_by_id($_GET,$id);
			}
			else
			{
			    	$data['all_users'] =  $this->user_model->get_assign_list_by_id($search='',$id);
			}
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$data = array(
						...
						'v_user_password' => $this->input->post('v_user_password'),
...
// edit user assign role
	...
	$data['user_list'] =  $this->usermodule_model->get_all_users($uid);
	$data['user_role'] =  $this->user_model->get_user_role();
			if($this->input->post('submit')){
				$this->form_validation->set_rules('hotel_id', 'Hotel', 'trim|required');
				$this->form_validation->set_rules('v_user_id', 'User', 'trim|required');
		...
		//edit user profile
		public function edituserprofile($id = 0){
			//echo $uid = $this->session->userdata('admin_id');
			 $uid = $this->session->userdata('admin_id');

			$data['user'] =  $this->user_model->get_vendor_by_id($uid);
			// $data['user_list'] =  $this->usermodule_model->get_all_users($uid);
			// $data['user_role'] =  $this->user_model->get_user_role();

			if($this->input->post('submit')){
				$this->form_validation->set_rules('v_fname', 'v_fname', 'trim|required');
				$this->form_validation->set_rules('v_lname', 'v_lname', 'trim|required');
				$this->form_validation->set_rules('v_mob', 'v_mob', 'trim|required');
				$this->form_validation->set_rules('v_email', 'Email', 'trim|required');
				$this->form_validation->set_rules('v_company', 'v_company', 'trim|required');
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$data = array(
						...
						'v_company' => $this->input->post('v_company'),
						'v_address' => $this->input->post('v_address'),
						'v_gst_number' => $this->input->post('v_gst_number'),
						'v_gst' => $this->input->post('v_gst'),
						'v_city' => $this->input->post('v_city'),
						'v_zip' => $this->input->post('v_zip'),
						
					);
					$data = $this->security->xss_clean($data);
					$result = $this->user_model->edit_user_profile($data, $id);
					if($result){
						$this->session->set_flashdata('msg', 'Record is Updated Successfully!');
						redirect(base_url('admin/users/edituserprofile'));
					}
				}
			}
			else{
