...
class Rooms extends MY_Controller {
	...
	public function TimmingAvilable(){
		...
		if($this->input->post('submit')){
			...
			//$data['all_hotels'] =  $this->hotel_model->get_all_hotels($id);
					$data['price']=$this->room_model->price_base_of_roomtypeid($hotel_id,$room_type_id);
					$data['all_details']=$this->room_model->all_room_details($hotel_id,$room_type_id);
					$data['rooms_available']=$this->room_model->get_roomtype_by_id($room_type_id);
					$data['view'] = 'admin/rooms/room_avilable';
					$this->load->view('admin/layout', $data);
	...
	public function roomtype(){
		...
		if($this->session->userdata('role') ==1){
			...
			$data['all_rooms'] =  $this->room_model->get_room_type_list_and_hotels_manager($hid,$id);
		// echo "<pre>";
		// print_r($data['all_rooms']);
			...
			{
				$data['all_rooms'] =  $this->room_model->get_room_type_list_and_hotels($_GET,$uid);
			}
			else
			{
				$data['all_rooms'] =  $this->room_model->get_room_type_list_and_hotels($search='',$uid);
			}
	...
	public function rooms_available_day(){
		$hotel_id=$_GET['hotel_id'];
		...
		$result=$this->room_model->rooms_available_day($data);
		
		
	}

	public function rooms_available_hour(){
		$hotel_id=$_GET['hotel_id'];
		$room_type_id=$_GET['room_type_id'];
		$date=$_GET['id'];
		$no_of_rooms=$_GET['no_of_rooms'];
		...
		$data=array($exp[0],$exp[1],$exp[2],$hotel_id,$room_type_id,$no_of_rooms);
		$result=$this->room_model->rooms_available_hour($data);
		
		
	}


	public function rooms_booked_day(){
		$hotel_id=$_GET['hotel_id'];
		$room_type_id=$_GET['room_type_id'];
		$date=$_GET['id'];
		...
		$result=$this->room_model->rooms_booked_day($data);
		var_dump($result);
		
	}

	public function rooms_booked_hour(){
		$hotel_id=$_GET['hotel_id'];
		$room_type_id=$_GET['room_type_id'];
		$date=$_GET['id'];
		$rooms_booked=$_GET['rooms_booked'];
		...
		$data=array($exp[0],$exp[1],$exp[2],$hotel_id,$room_type_id,$rooms_booked);
		$result=$this->room_model->rooms_booked_hour($data);
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array(
					...
					'no_of_rooms_hour' =>$this->input->post('no_of_rooms_hour'),
