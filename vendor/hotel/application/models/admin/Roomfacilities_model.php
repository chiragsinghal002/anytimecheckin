<?php
	class Roomfacilities_model extends CI_Model{
		...
		public function get_all_facility($search=false,$id)
		{
			$this->db->select('*');
		    $this->db->from('facility_id');
            if(!empty($search) && $search['status']!="")
            {
	    	    $this->db->where(array('facility_id.status' => $search['status']));
            }
            if(!empty($search) && $search['facility_title']!="")
            {
	    	    $this->db->where(array('facility_id.facility_name' => $search['facility_title']));
            }
	    	
             $this->db->where(array('for_room_type' => '1','vendor_id' => $id));
	    	$query = $this->db->get();		
			return $result= $query->result_array();
		}
