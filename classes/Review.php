...
{ 
  ...
  // avarag rating 
  public  function AvarageRating($hotel_id) {
    
    $result = $this->db->query("SELECT AVG(user_rating) FROM user_reviews WHERE `hotel_id` = '$hotel_id'") or die(mysqli_error($this->db)); 
  ...
  public  function FetchReviews($hotel_id) {
    $created=date('Y-m-d h:i:s'); 
    // echo $hotel_id; 
     // echo "SELECT user_reviews.*,vendor_reviews.* from user_reviews INNER JOIN vendor_reviews ON  user_reviews.review_id=vendor_reviews.ureview_id where user_reviews.hotel_id =$hotel_id and vendor_reviews.vhotel_id=$hotel_id";die;  
    
    $result = $this->db->query("SELECT user_reviews.*,vendor_reviews.*,user_registration.fname,user_registration.lname from user_reviews LEFT JOIN vendor_reviews ON  user_reviews.review_id=vendor_reviews.ureview_id INNER JOIN user_registration ON user_reviews.user_id=user_registration.user_id where user_reviews.hotel_id =$hotel_id and vendor_reviews.vhotel_id=$hotel_id") or die(mysqli_error($this->db)); 
    $result1 = mysqli_num_rows($result);
    if($result1>0){
     while($row = mysqli_fetch_array($result)){
      $data[]=$row;
...
public  function FetchvendorReviews($hotel_id) {
    $created=date('Y-m-d hh:ii:ss'); 
        
    $result = $this->db->query("SELECT * from vendor_reviews INNER JOIN user_reviews ON user_reviews.hotel_id = vendor_reviews.vhotel_id  INNER JOIN ci_hotels ON ci_hotels.hotel_id = user_reviews.hotel_id INNER JOIN ci_vendors ON ci_vendors.v_id = vendor_reviews.vendor_id where user_reviews.hotel_id = '$hotel_id'") or die(mysqli_error($this->db)); 
    $result1 = mysqli_num_rows($result);
    if($result1>0){
     while($row = mysqli_fetch_array($result)){
      $data[]=$row;
...
// manager review
public  function FetchManagerReviews($hotel_id) {
    $created=date('Y-m-d hh:ii:ss'); 
        
    $result = $this->db->query("SELECT * from vendor_reviews INNER JOIN user_reviews ON user_reviews.hotel_id = vendor_reviews.vhotel_id  INNER JOIN ci_hotels ON ci_hotels.hotel_id = user_reviews.hotel_id INNER JOIN ci_v_assign_role ON ci_v_assign_role.v_user_id = vendor_reviews.vendor_id INNER JOIN ci_v_users ON ci_v_users.v_user_id = ci_v_assign_role.v_user_id where user_reviews.hotel_id = '$hotel_id'") or die(mysqli_error($this->db)); 
    $result1 = mysqli_num_rows($result);
    if($result1>0){
     while($row = mysqli_fetch_array($result)){
      $data[]=$row;
